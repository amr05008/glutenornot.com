...
- **Live evals spend real credits — one FULL run per PR**: `RUN_LIVE_EVALS=1 node --env-file=.env node_modules/vitest/vitest.mjs run --root web tests/api/evals` samples each case once (42 Opus calls, well under $1) — iterate with that and `-t`. `FULL=1` is the merge gate (2× safe / 5× adversarial, 165 calls ≈ $1.50 with the cached prompt, ≈ $6 uncached): run it once, before merge, and a human approves the command (`.claude/settings.json` asks on `RUN_LIVE_EVALS` and `--env-file`); a second FULL run within an hour is refused unless `FORCE=1`. A review/grill agent never runs live evals — a review shouldn't spend. (2026-09-16: nine unguarded FULL runs in 50 minutes emptied the org's prepaid credits and took the live app down for two hours.)
