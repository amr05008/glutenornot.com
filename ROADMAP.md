...
- [x] A label cut off at the top or bottom of its ingredient list can't come back `safe` (`applyIngredientListGate`, `api/analyze.js`, decision 005, PR #31, 2026-09-22): from the jev-sandbox truncation test (`plans/jev-findings-handoff-2026-09-18.md` finding 5 — Claude called 39 of 600 half-cut gluten labels `safe`, every one because the cut removed the gluten word). A photographed label needs an ingredients heading starting its line (the start) and a full stop or allergen line after it (the end) before `safe`; otherwise `caution` with retake copy. Replay with a heading added: 13/13 end cuts caught (the database text has no headings, so the start-cut replay isn't selective). Real photos (`test-cases/`, Vision): 8/8 complete label reads pass (6 iPhone photos, 2 grocery screenshots); 4/4 top crops with text and 2/2 real bottom crops caught; side crops leak (known). Grilled twice. `list_gate` on `scan` records each downgrade. Code-only: no prompt change, no eval spend, no iOS build
- [ ] Read `list_gate` ~2026-10-06: share of OCR label scans where it fired, split `no_heading` / `no_end`. If it withholds a large share of would-be `safe` verdicts, suspect a heading format the pattern misses — reproduce with tester photos, never by logging text. Side-cut leak: consider a prompt rule for words cut at the frame edge (needs a live eval run)
