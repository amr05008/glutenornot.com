...
- [x] Honor an explicit gluten-free label claim (`api/analyze.js` prompt, decision 003, `plans/gf-label-claim-2026-08-28.md`): a regulated claim (FDA 21 CFR 101.91 / EU 828/2014, <20 ppm) now lifts ambiguous-ingredient cautions (natural flavors, maltodextrin, modified starch, spices, hydrolyzed protein of unstated source) to `safe` with the label named as the reason; oats (unless certified — relaxed by decision 004 below), a listed gluten source, and may-contain / shared-equipment advisories still win; "hydrolyzed vegetable protein" narrowed to unstated-source only. Gated by a 16-case live eval in the repo (`web/tests/api/evals/`, `RUN_LIVE_EVALS=1`): baseline 8/16 → 16/16 on two runs, zero false-safe. `gf_claim_present` on OCR `scan` events measures the effect (2026-08-28, PR #23)
- [ ] Read the effect (plan step 9, ~2026-09-25, weekly review — **group by `list_gate` too**: from 2026-09-22 the cut-off-list gate downgrades some Claude `safe`s, decision 005; **and end the window at PR #32's deploy, 2026-09-24 14:35 UTC**, or split by `caution_reason IS NULL` — decision 006 changes the caution rule from then; the 2026-09-23 baseline is in `plans/verdict-calibration-2026-09-23.md`): caution share among `gf_claim_present = true` OCR scans should fall sharply, `false` roughly unchanged; if labeled cautions stay high, sample explanation *reasons* (never content) and revisit T1/T3. Then close the plan with a CLOSED header. **Known contaminant (2026-08-28, first production event with the flag):** a frame holding two products — a wheat fig bar beside a certified-GF cracker box — read `gf_claim_present = true` with verdict `unsafe`. The verdict was right (the claim belonged to the other product; wheat was listed); the flag is a presence regex over the whole OCR text, so multi-product shelf shots will inflate `true` among non-safe verdicts. Read `true × unsafe` as "shelf shot", not as a rule failure
- [x] A whole-product gluten-free claim now covers oats, on both paths (decision 004, `.claude/decisions/004-gf-claim-covers-oats.md`, 2026-09-16): a GFCO-certified fig bar came back caution twice — by photo because the mark is on the front and the ingredients on the back (the T2 "certified clears oats" exception was unreachable in one frame), and by barcode because the path still ran the pre-003 "all oats caution" rule and read the label-vs-auto-derived-gluten-tag pair as a conflict. Fix: T2 flipped (the regulation holds a labeled product's oats to the same 20 ppm; explanation adds an avenin clause), the claim block ported to `api/barcode.js` with the HVP narrowing (T5 reversed), `assessGlutenSignal` note rewritten so the label wins, caution copy for unlabeled oats points at the barcode. OCR eval 22 → 27 cases (5 flipped to safe; 23–27 new, incl. a Spanish claim + avena and a two-product frame), new 15-case barcode eval (`barcode-gf-claim-cases.js`, incl. the grill cases: label + gluten tag + no oats → caution; `en:contains-gluten` and free-text "very low gluten" → never safe, even beside a `no-gluten` tag; wheat/barley/rye-specific tag + label + oats → caution; unrecognized free text "not suitable for celiacs" → never safe, and unrecognized positive free text never demotes a labeled product; safe-with-oats explanations must end with the one prescribed avenin-caveat sentence, asserted verbatim): 42/42 live, zero false-safe on the final prompts. `gf_label_present` on barcode `scan` events is the measurement. Two grills (mine, then two rounds of Pi/astra on PR #29) caught five real seams before ship: the allowlist dropped adverse and unrecognized free-text tags to the same context as "no label", the label-wins note fired on a grain-specific tag, the barcode note explained a gluten tag away as "oats" without checking for oats, and an ingredient-level "gluten-free oats" + natural flavors was talked into safe 3/5 until the prompt said the ingredient-level claim does NOT lift the verdict. The back-panel photo of a certified product still returns caution (no whole-product claim in frame) — the barcode is the honest path for small packages
- [x] A label cut off at the top or bottom of its ingredient list can't come back `safe` (`applyIngredientListGate`, `api/analyze.js`, decision 005, PR #31, 2026-09-22): from the jev-sandbox truncation test (`plans/jev-findings-handoff-2026-09-18.md` finding 5 — Claude called 39 of 600 half-cut gluten labels `safe`, every one because the cut removed the gluten word). A photographed label needs an ingredients heading starting its line (the start) and, after every heading, a line-ending full stop (the end) before `safe`; otherwise `caution` with retake copy. Replay with a heading added: 13/13 end cuts caught (the database text has no headings, so the start-cut replay isn't selective). Real photos (`test-cases/`, Vision): 8/8 complete label reads pass (6 iPhone photos, 2 grocery screenshots); 4/4 top crops with text, 2/2 bottom crops and 2/2 side crops caught (all pinned in `web/tests/fixtures/real-label-ocr.json`). A bottom-cut sweep over the real lists (34 cuts) catches all but 3 named leaks, pinned as a test. Grilled by an in-session agent and six rounds of an Opus 5.5 Herdr session (final: SHIP). `list_gate` on `scan` records each downgrade. Code-only: no prompt change, no eval spend, no iOS build
- [ ] Read `list_gate` ~2026-10-06: share of OCR label scans where it fired, split `no_heading` / `no_end`, by `detected_language` (a missed heading format shows up as one language), and by `ocr_chars` (a `no_heading` on a 600+ char read is more likely a missed heading than a cut). If it withholds a large share of would-be `safe` verdicts, suspect a heading format the pattern misses — reproduce with tester photos, never by logging text. Side-cut leak: consider a prompt rule for words cut at the frame edge (needs a live eval run)
- [x] Relaxing the ambiguous-ingredient list itself when *no* claim is present (decision 006, `.claude/decisions/006-caution-means-a-specific-reason.md`, `plans/verdict-calibration-2026-09-23.md`, 2026-09-23): caution now means a specific reason to worry. Every caution names one `caution_reason` (`oats`, `may_contain`, `conflict`, `undeclared_source`, `incomplete`, `other`); unnamed natural flavors, spices, maltodextrin, dextrin, modified starch, glucose syrup, caramel color and HVP of unstated source are no reason on their own outside meat and poultry products (US/EU/UK/CA/AU law makes wheat be named). Same rule on both paths; `caution_reason` on `scan` events measures it. New 44-case calibration eval (`calibration-cases.js`, both paths); claim-rule cases that leaned on natural flavors re-derived with plain oats. Baseline: 68% photo / 65% barcode caution over the 60 days to 2026-09-23
- [ ] Day-28 read (~2026-10-22, plan Task 9; PR #32 deployed 2026-09-24 14:35 UTC): caution share of readable label scans (target ≤ 40%), `other` ≤ 10% of cautions, the `incomplete` share (sizes the retake screen, Part B), and any user false-safe report
...
- [ ] **Jev fast path** (decision 007, `plans/jev-fast-path-2026-09-24.md`). PR #35 merged 2026-09-25, and **shadow mode has been live since 2026-09-25**; tracked on the PostHog dashboard "Jev fast path: production bake-off". TypeSafe's Jev answers first on Open Food Facts barcodes it's sure of (Jev itself ~0.2 s against Opus's ~3 s; the response adds the database lookup); Claude runs on every scan and audits each settled verdict (`engine_audit`). Rollout: a shadow day, then Stage 1 (`unsafe` served), then Stage 2 (`full`) once ≥ 50 shadowed Jev-`safe` audits over ≥ 3 weeks show zero Claude disagreements **and** the product-name meat check has shipped (below), with the PostHog tripwire armed. **Precondition for Stage 2 (own PR):** a local, whole-word meat/poultry check of the product name that blocks a fast `safe` (Claude reads the name, Jev doesn't; the name never goes to TypeSafe; word boundaries — "ham" is inside "Champions"). Follow-up PR: fix the shared `isGlutenFamilyTag` (case-sensitive, English-only; misses `en:Glutine` and ~12 other forms), which changes Claude's inputs and needs its own eval run
...
- [ ] **Parked (low priority): retry upstream Claude connection-timeouts (Cloudflare `522`) in `callClaude`.** PR #14 retries Anthropic *overloads* (429/529), but a `522` "connection timed out" (Cloudflare in front of `api.anthropic.com`) currently surfaces straight to the client as `CLAUDE_ERROR` → 503. Observed once at **2026-07-22T12:13:23Z** (1 scan, 1 user, self-recovered, surrounded by successes) — root-caused from Vercel runtime logs. **Revisit trigger:** if `claude_error` frequency climbs from a rare one-off to a pattern, or next time `_utils.js` is open anyway. (2026-07-22)
- [ ] **Parked (cosmetic): replace `url.parse()` with the WHATWG `new URL()` API.** Fires a Node `DEP0169` DeprecationWarning on every `/api/health` hit (22×+ since 2026-06-18; Vercel files it under "error" level though it's only a warning), cluttering the runtime-error view. Harmless — no user/functional impact. **Revisit trigger:** fold in next time `/api/health` or its deps are touched. (2026-07-22)
...
- [x] Instrument OCR captures: `image_kb` + `ocr_chars` on `scan`/OCR `scan_failed` events — decides the capture-assist fork (aiming vs blur; **decided 2026-08-13: aiming, not blur — but the plan closed without shipping a client fix, goal already met at 7.4%**) via failure-vs-success `image_kb` distributions; see `plans/ocr-capture-assist-2026-07-18.md` (PR #17, 2026-07-18)
...
- [x] Hard safety floor: an OCR extraction under 100 chars can never return `safe` (`applySafeVerdictFloor`, `api/analyze.js`) — threshold is the 10th percentile of successful extractions (p10 97.6, median 725), not a round guess; one 3-char read returned `safe` on 2026-07-19 and every other sub-threshold read had degraded to caution on its own (2026-08-13, PR #22)
- [x] Add `app_version` (via the `X-Client-Version` header) + `model` to scan events so releases and model swaps are attributable — every event previously carried only `$lib_version: posthog-node`, the SDK's version, which is why 1.4.1's capture work could not be measured (2026-08-13, PR #22; ships with iOS 1.4.2)
- [x] Exclude App Store review traffic from scan analytics **by rule, not identity** (PostHog cohort 481139 + inline predicate; `reports/weekly-snapshot/README.md`) — it was 14 of 35 `ocr_failed` events in the 30 days to 2026-08-13 and recurs after every submission, since `distinct_id` is a hash of the client IP (2026-08-13)
- [ ] Auto-retry with backoff (1–2 attempts) on `network`/`timeout` so a slow-but-alive connection still lands without a user retry (deferred from PR #15; not currently scheduled — connectivity work, independent of the closed capture-assist plan)
- [x] Fire a **client-side** `scan_failed` (reason: `timeout`/`network`) via the `POST /api/track` beacon — previously a client timeout logged only a Sentry `warning` and the server logged a *success*, so connectivity failures were nearly invisible in PostHog (made the 2026-07-11 incident hard to triage) (capture-assist Phase 2, 2026-07-19; ships with iOS 1.4.0)
- [ ] **Weak-signal OCR upload** (`plans/weak-signal-upload-2026-08-28.md`) — from a 2026-08-28 field incident on 2-bar LTE: three attempts for one verdict, and the two failed attempts left *no* telemetry (user-cancel is an `AbortError` the client dropped before Sentry or the beacon fired). The OCR upload is p50 408 KB raw / ~545 KB on the wire (1024px @ JPEG 0.7) — 10–45 s on a rural uplink before the server even starts; the 30 s copy then told the user to *restart* it. **Phase A done 2026-08-28**: `ocr_ms`/`claude_ms`/`total_ms` on `scan` (the server leg is measured for the first time — decision 002's Opus-latency revisit trigger now has data), `cancelled` + `elapsed_ms` accepted by the beacon. **Phase C done 2026-08-28** (client, unreleased): the OCR request moved from `fetch` to XHR so the screen shows "Uploading photo… N%" and switches to "Reading ingredients…" once iOS reports the body sent (handed to the network stack — it can run a few seconds ahead of the server on a slow uplink); the slow copy is per phase with its own clock (30 s uploading: "Slow connection — still uploading…"; 20 s server-side: a non-prescriptive "still working" — the first cut showed "cancel and try again" the instant a long upload landed, caught by /grill); the screen beacons `cancelled` + `elapsed_ms` on user cancel and `interrupted` (no elapsed) when the app goes to the background mid-scan (moved from resume to the background transition after /grill — resume raced the dead socket and the 60 s timer; a transient `inactive` no longer kills a scan), on both scan paths (timeout/network carry `elapsed_ms` too). Also closed by /grill: a Cancel during the photo resize now aborts (the request used to go out anyway), and RN's XHR `timeout` event is handled (it used to hang the spinner). **Phase B done 2026-08-28**: JPEG 0.7→0.6 by measurement on nine real labels (table in the plan; 0.5 lost 5.1% on the densest label); ~15% fewer bytes, so T2 (multipart, ~25%) is the next lever as its own PR. **Phase D: iOS 1.4.3 submitted 2026-08-28** (PR #24 merged `3adef89`, tag `v1.4.3`); TestFlight scans showed `image_kb` 339/347 vs the 0.7-era p50 of 408 and server `total_ms` 3.4–4.2 s. **Remaining**: the D2 read ~2026-09-11 (cancelled/interrupted counts + `elapsed_ms`, `image_kb` p50 ≈ 340 among 1.4.3, `total_ms` p50, `ocr_failed` from 1.4.3 builds), then close the plan; the `cancelled`/`interrupted` beacons and the 3G copy were not exercised on a device before submission, so the first real `cancelled` event is their verification.
...
- [x] Improve "not found" UX → **Barcode recovery** (`plans/barcode-recovery-2026-09-05.md`, PR #25 merged 2026-09-05, grilled twice): a barcode dead end — 404 `not_found`, or a 200 the server now marks `result_reason: "missing_context"` (no ingredient/allergen context; set only on that branch, never from prose) — lands on a persistent neutral state ("Product not found" / "Not enough information") instead of a toast or an amber verdict, and one tap opens a photo-only camera (barcode detection off for the whole flow, labelled exit, same control positions; exit silently ignores the same code for 60 s). Those states never touch history / scan count / rating. Measured by the content-free `barcode_recovery` funnel (`POST /api/recovery`; flow_id + reason + stage + bounded enums; contract in `api/ANALYTICS.md`) — primary read at day 14 after the public release: unique flows with a displayed *label* result ÷ unique flows that started a photo; pilot target ≥50%. The api half is live; the client half shipped as **iOS 1.5.0** (build 2 submitted 2026-09-05, rejected under 5.1.1(iv) for the unrelated camera pre-permission button; build 3 resubmitted 2026-09-06 via PR #26; TestFlight smoke on build 1 proved every funnel stage on device). The Open Food Facts "add this product" link was deliberately not built (no submit-product UI this release).
- [ ] Barcode recovery, step 2 (gated, not started): bounded enrichment of data-empty Open Food Facts hits via USDA/UPCitemdb — feasibility on a small consented/public fixture set first, then a default-off server flag with a ≤2 s budget and the end-to-end waterfall deadline above as a prerequisite. "Skip enrichment; photo recovery is the better trade-off" is a valid outcome (plan §8).
