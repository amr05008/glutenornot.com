...
- [x] A cut-off label can't come back `safe` (`applyIngredientListGate`, `api/analyze.js`, 2026-09-22): from the jev-sandbox truncation test (`plans/jev-findings-handoff-2026-09-18.md` finding 5 — Claude called 39 of 600 half-cut gluten labels `safe`, every one because the cut removed the gluten word). A photographed label needs an ingredients heading (the start) and a full stop after it (the end) before `safe`; otherwise `caution` with retake copy. Replay: blocks all 39. Real photos (`test-cases/`): 8/8 complete label reads pass (6 iPhone photos, 2 grocery screenshots); 5/5 top-cropped and 3/3 bottom-cropped reads caught; a side crop leaked (full stop from the address line below). `list_gate` on `scan` measures the cost. Code-only: no prompt change, no eval spend, no iOS build
