...
- [x] Instrument OCR captures: `image_kb` + `ocr_chars` on `scan`/OCR `scan_failed` events — decides the capture-assist fork (aiming vs blur; **decided 2026-08-13: aiming — ships as framing guidance in 1.4.3**) via failure-vs-success `image_kb` distributions; see `plans/ocr-capture-assist-2026-07-18.md` (PR #17, 2026-07-18)
...
- [ ] Auto-retry with backoff (1–2 attempts) on `network`/`timeout` so a slow-but-alive connection still lands without a user retry (deferred from PR #15; queued for 1.4.3)
